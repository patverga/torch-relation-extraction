...
local MM_fixed, parent = torch.class('nn.MM_fixed', 'nn.Module')
...
function MM_fixed:__init(transA, transB)
  parent.__init(self)
...
function MM_fixed:updateOutput(input)
...
function MM_fixed:updateGradInput(input, gradOutput)
  assert(#input == 2, 'input must be a pair of tensors')
  local a, b = table.unpack(input)
  if not self.gradInput[1] or not self.gradInput[2] then
    self.gradInput = {a:clone(), b:clone() }
  else
