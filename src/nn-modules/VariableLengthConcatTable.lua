local VariableLengthConcatTable, parent = torch.class('nn.VariableLengthConcatTable', 'nn.Container')

function VariableLengthConcatTable:__init()
   parent.__init(self)
   self.modules = {}
   self.output = {}
end

function VariableLengthConcatTable:updateOutput(input)
...
function VariableLengthConcatTable:updateGradInput(input, gradOutput)
...
function VariableLengthConcatTable:accGradParameters(input, gradOutput, scale)
   scale = scale or 1
   for i,module in ipairs(self.modules) do
...
function VariableLengthConcatTable:accUpdateGradParameters(input, gradOutput, lr)
   for i,module in ipairs(self.modules) do
      module:accUpdateGradParameters(input, gradOutput[i], lr)
   end
end

function VariableLengthConcatTable:zeroGradParameters()
   for _,module in ipairs(self.modules) do
      module:zeroGradParameters()
   end
end

function VariableLengthConcatTable:__tostring__()
   local tab = '  '
   local line = '\n'
   local next = '  |`-> '
   local ext = '  |    '
