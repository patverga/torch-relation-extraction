local ConcatTable_hacked, parent = torch.class('nn.ConcatTable_hacked', 'nn.Container')

function ConcatTable_hacked:__init()
   parent.__init(self)
   self.modules = {}
   self.output = {}
end

function ConcatTable_hacked:updateOutput(input)
...
local function backward(self, method, input, gradOutput, scale)
   ...
   if isTable then
      for i,module in ipairs(self.modules) do
         ...
         -- autograd doesnt handle this correctly
         ...
         if i == 1 then
            ...
            retable(self.gradInput, currentGradInput,
               function(t, k, v)
                  if t[k] and t[k]:dim() > 0 and v and v:dim() > 0 then
                     t[k]:add(v)
                  else
                     t[k] = v:clone()
                  end
...
function ConcatTable_hacked:updateGradInput(input, gradOutput)
   return backward(self, 'updateGradInput', input, gradOutput)
end

function ConcatTable_hacked:backward(input, gradOutput, scale)
   return backward(self, 'backward', input, gradOutput, scale)
end

function ConcatTable_hacked:accGradParameters(input, gradOutput, scale)
   scale = scale or 1
   for i,module in ipairs(self.modules) do
...
function ConcatTable_hacked:accUpdateGradParameters(input, gradOutput, lr)
   for i,module in ipairs(self.modules) do
      self:rethrowErrors(module, i, 'accUpdateGradParameters', input, gradOutput[i], lr)
   end
end

function ConcatTable_hacked:__tostring__()
   local tab = '  '
   local line = '\n'
   local next = '  |`-> '
   local ext = '  |    '
