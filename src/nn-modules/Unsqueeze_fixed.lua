local Unsqueeze_fixed, parent = torch.class('nn.Unsqueeze_fixed', 'nn.Module')

local function _assertTensor(t)
   assert(torch.isTensor(t), "This module only works on tensor")
end

function Unsqueeze_fixed:__init(pos, numInputDims)
   parent.__init(self)
   self.pos = pos or error('the position to insert singleton dim not specified')
   self:setNumInputDims(numInputDims)
end

function Unsqueeze_fixed:setNumInputDims(numInputDims)
   self.numInputDims = numInputDims
   return self
end

function Unsqueeze_fixed:updateOutput(input)
   _assertTensor(input)
   local actualPos = self:_getActualPosition(input)
   if not self.output or self.output:dim() == 0 or torch.type(self.output) ~= torch.type(input) then
...
function Unsqueeze_fixed:updateGradInput(input, gradOutput)
   _assertTensor(input)
...
function Unsqueeze_fixed:__tostring__()
   return torch.type(self)..'(dim ' .. self.pos .. ')'
end

function Unsqueeze_fixed:_getActualPosition(input)
