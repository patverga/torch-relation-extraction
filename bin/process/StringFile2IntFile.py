...
-s <throw away relations longer than this> -c <use char tokens (default is use words)> -d <double vocab depending on if [A1 rel A2] or [A2 rel A1]>'
    ...
    # if directory was provided, process all files in the dir
    in_files, out_files = ([in_file+'/'+f for f in os.listdir(in_file)], [out_file+'/'+f for f in os.listdir(in_file)]) \
        if os.path.isdir(in_file) else ([in_file], [out_file])
    for in_f, out_f in zip(in_files, out_files):
        # memory map all the data and return processed lines
        print 'Processing lines and getting token counts from ' + in_f
