...
while read REL; do
  ...
  do
    ...
    RESULT=`$SCORE_SCRIPT $TMP_RESPONSE $key slots=$slotlist nocase anydoc | grep -e 'F1: ' -e 'Precision: '`
    FSCORE=`echo $RESULT | grep 'F1: ' | sed 's#.*F1:\(.*\)#\1#'` # | sed 's/^[ \t]*//;s/[ \t]*$//'`
    P=`echo $RESULT | grep 'Precision: ' | sed 's#.*Precision:\(.*\)#\1#'` # | sed 's/^[ \t]*//;s/[ \t]*$//'`
    if [[ "$FSCORE" == "NaN" ]]
    then
        FSCORE=0.0
    fi
    FSCORE=`echo "$FSCORE + ($P * $P_BIAS)" | bc`
    #echo $'\r'"F1: $FSCORE"
